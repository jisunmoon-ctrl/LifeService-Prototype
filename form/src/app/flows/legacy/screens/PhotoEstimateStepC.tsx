...
import { ChevronDown, ChevronUp, Plus, X, Camera, Grid3x3 } from "lucide-react";

interface PhotoEstimateStepCProps {
...
export default function PhotoEstimateStepC({ onNavigate }: PhotoEstimateStepCProps) {
  const [activeTab, setActiveTab] = useState<'photos' | 'items'>('photos');
  ...
  return (
    <div className="bg-[#eaebef] relative size-full">
      ...
      <div className="absolute top-0 left-0 right-0 z-10">
        ...
        {/* Tab Navigation */}
        <div className="bg-white flex border-b border-[var(--border-neutral)]">
          <button
            onClick={() => setActiveTab('photos')}
            className={`flex-1 py-3 relative transition-default ${
              activeTab === 'photos'
                ? 'text-[var(--fg-brand)] font-semibold'
                : 'text-[var(--fg-weak)]'
            }`}
          >
            <div className="flex items-center justify-center gap-2">
              <Camera className="size-4" />
              <span className="text-body-14">사진 촬영</span>
              {totalPhotos > 0 && (
                <span className="bg-[var(--bg-brand)] text-white text-detail-10 px-1.5 py-0.5 rounded-full">
                  {totalPhotos}
                </span>
              )}
            </div>
            {activeTab === 'photos' && (
              <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-[var(--bg-brand)]"></div>
            )}
          </button>
          <button
          ...
          >
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-body-14">큰 짐 입력</span>
      ...
      {/* 바디 영역 */}
      <div className="absolute left-0 right-0 top-[138px] bottom-[96px] overflow-y-auto">
        {activeTab === 'photos' ? (
          // 사진 촬영 탭
                ...
                <div className="flex items-center justify-between px-2 py-2">
                  {editingRoomId === room.id ? (
                    ...
                    <h3
                      ...
                      className="text-body-16 font-semibold text-[var(--fg-neutral)] cursor-pointer"
                    >
                      {room.name}
                    </h3>
                  )}
